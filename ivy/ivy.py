...
class IvyClient:
    ...
    def __init__(self, ip, port, client_socket,
        ...
        self.status = IvyClient.NOT_INITIALIZED
    ...
    def start_init(self, agent_name):
        ...
        :except IvyIllegalStateError: if the method has already been called
          once
        """
        if self.status != IvyClient.NOT_INITIALIZED:
            raise IvyIllegalStateError
        self.agent_name = agent_name
        self.status = IvyClient.INITIALIZATION_IN_PROGRESS
    ...
    def end_init(self):
        ...
        """
        if self.status == IvyClient.INITIALIZED:
            raise IvyIllegalStateError
        debug('Client:%r: Initialization ended', self)
        self.status = IvyClient.INITIALIZED
    ...
    def add_regexp(self, regexp_id, regexp):
        ...
        """
        if self.status not in (IvyClient.INITIALIZATION_IN_PROGRESS, IvyClient.INITIALIZED):
    ...
    def send_msg(self, msg):
        ...
        if self.status != IvyClient.INITIALIZED:
            return
        debug('Client:%r: Searching a subscription matching msg %r', self, msg)
    ...
    def get_clients(self):
        ...
        try:
            return [c.agent_name for c in self._clients.values()
                    if c.status == IvyClient.INITIALIZED]
        finally:
            self._global_lock.release()
    ...
    def _get_client(self, ip, port, socket=None,
        ...
        try:
            ...
            if agent_id and agent_id in [c.agent_id for c in self._clients.values()]:
                return None
            return self._clients.setdefault((ip, port), IvyClient(ip, port, socket,
                                                                  agent_id, agent_name))
        finally:
            self._global_lock.release()
...
class IvyTimer(threading.Thread):
    ...
    **Please note:** ``start()`` starts a new thread; if the same function is
    ...
    def run(self):
        ...
        log('IvyTimer %s terminated', self._id)
